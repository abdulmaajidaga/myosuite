...
try:
    from cvae import CVAE, KinematicDataset
except ImportError:
    print("Error: Could not import CVAE/KinematicDataset from cvae.py")
    sys.exit(1)
...
def plot_latent_tsne(model, dataset):
    ...
    plt.title("CVAE Latent Space t-SNE (Cluster Separation)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.show()

def plot_trajectory_dynamics(dataset):
    print("Generating 3D Wrist Trajectory Dynamics (X vs Y vs Time)...")
    ...
    # Collect a few samples of each
    for i in range(n):
        x, c = dataset[i]
        # x is Tensor(6301)
        # Trajectory part is first 6300
        traj = x[:6300].numpy()
        
        # Reshape to (100, 63) -> 100 frames, 21 markers * 3
        traj_reshaped = traj.reshape(100, -1)
        
        if c.item() > 0.9:
            if len(healthy_samples) < 10: healthy_samples.append(traj_reshaped)
        else:
            if len(stroke_samples) < 10: stroke_samples.append(traj_reshaped)
    ...
    # Plot Healthy (Green)
    for traj in healthy_samples:
        # Indices 0, 1, 2 are typically Wrist X, Y, Z (based on visualise_cvae.py)
        # We plot X vs Y vs Time
        wrist_x = traj[:, 0]
        wrist_y = traj[:, 1]
        
        ax.plot(wrist_x, wrist_y, time, c='green', alpha=0.4, linewidth=1)

    # Plot Stroke (Red)
    for traj in stroke_samples:
        wrist_x = traj[:, 0]
        wrist_y = traj[:, 1]
        
        ax.plot(wrist_x, wrist_y, time, c='red', alpha=0.8, linewidth=1.5)
        
    # Legend
    ax.plot([], [], [], c='green', label='Healthy')
    ax.plot([], [], [], c='red', label='Stroke')
    
    ax.set_xlabel("Wrist X Position")
    ax.set_ylabel("Wrist Y Position")
    ax.set_zlabel("Time (Frames)")
    ax.set_title("Wrist Trajectory Dynamics (CVAE Data)")
    ax.legend()
    plt.show()
