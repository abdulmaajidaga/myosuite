...
# Defined Ranges for TransformerVAE
PARAM_GRID = {
...
def load_dataset():
    ...
    return transformer_vae.TransformerDataset(stroke, healthy, scores, OUTPUT_DIR)
...
def evaluate_physics(model, pca, scaler):
    ...
    for _ in range(n_samples):
        # Use fc_mu output size for latent dim
        lat_dim = model.fc_mu.out_features
        z = torch.randn(1, lat_dim)
...
def run_trial(dataset, params, pca, scaler):
    ...
    transformer_vae.LATENT_DIM = params['LATENT_DIM']
    model = transformer_vae.TransformerVAE().to(device)
...
def main():
    ...
    print(f"\nStarting Auto-Tuning TransformerVAE ({N_TRIALS} trials)...")
    ...
    for i in range(N_TRIALS):
        ...
        try:
            model, score, drift, jerk = run_trial(ds_trans, params, pca, scaler)
            print(f"  -> Score: {score:.4f} | Drift: {drift:.2f} | Jerk: {jerk:.0f}")
            
            if score < best_score:
                best_score = score
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_TransformerVAE.pth"))
                with open(os.path.join(OUTPUT_DIR, "best_TransformerVAE_config.json"), 'w') as f:
