"""
3D kinematic animation of raw MHH CSV recordings.

What it does:
  Reads a raw MoCap CSV (2-row multi-level header: marker names + X/Y/Z),
  applies a 10 Hz low-pass Butterworth filter, computes virtual joint centres
  (shoulder, elbow, wrist), and renders a real-time 3D stick-figure animation.

Input:
  - data/kinematic/healthy/*.csv or data/kinematic/stroke/*.csv
    (2-row header: marker names row + axis names row, 200 Hz, mm)

Output:
  - Interactive matplotlib 3D animation window (no file saved)

Usage:
  python scripts/viz/render/play_kinematics.py
  (edit the filepath variable at the bottom of the script to select a file)
"""
import os
import sys
import pandas as pd
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(get_path("data_healthy"), "05_12_1.csv")
