"""
Advanced raw-MoCap analysis — 10 exploratory figures from original recordings.

What it does:
  Loads raw MHH CSV recordings and pre-computed phase indices, then generates
  10 diagnostic figures covering kinematic heatmaps, parallel coordinates,
  time-normalised velocities, 2D/3D trajectories, boxplots, cyclograms,
  and efficiency scatter plots.

Input:
  - data/kinematic/healthy/*.csv or data/kinematic/stroke/*.csv
    (2-row multi-level header, 200 Hz, mm — raw recordings)
  - output/phase_indices/healthy_phase_indices.json or stroke_manual_phase_indices.json
    (pre-computed by phase_selector.py)

Output (all in output/analysis/):
  - analysis_01_heatmap.png, analysis_02_parallel.png, analysis_03_velocity.png
  - analysis_04_trajectories.png, analysis_05_poses.png, analysis_06_boxplots.png
  - analysis_07_paths3d.png, analysis_08_cyclograms.png
  - analysis_09_advanced.png, analysis_10_efficiency.png

Usage:
  python scripts/viz/explore/advanced_analysis.py --healthy
  python scripts/viz/explore/advanced_analysis.py --stroke
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    if args.dataset == 'healthy':
        ...
        output_dir = os.path.join(get_path("output_analysis"), "healthy")
        os.makedirs(output_dir, exist_ok=True)
        json_path = os.path.join(get_path("output_phase_indices"), "healthy_phase_indices.json")
    else:
        data_dir = get_path("data_stroke")
        output_dir = get_path("output_analysis")
        json_path = os.path.join(get_path("output_phase_indices"), "stroke_phase_indices.json")
