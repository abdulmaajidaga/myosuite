"""
Interactive phase annotation tool — manually label Pick/Drink/Place frame indices.

What it does:
  Displays each raw CSV recording as an animated velocity profile with peak
  detection. You click to confirm or override 5 phase-boundary frame indices
  (start, pick, drink, place, end). Saves all annotations incrementally to a
  JSON file so you can resume across sessions.

Input:
  - data/kinematic/healthy/*.csv or data/kinematic/stroke/*.csv
    (2-row multi-level header, 200 Hz, mm — raw recordings)
  - output/phase_indices/healthy_phase_indices.json (existing annotations, if any)

Output:
  - output/phase_indices/manual_phase_indices.json      (healthy)
  - output/phase_indices/stroke_manual_phase_indices.json (stroke)
  - output/analysis/velocity_phases_comparison_manual.png — summary image

Usage:
  python scripts/viz/explore/phase_selector.py --healthy
  python scripts/viz/explore/phase_selector.py --stroke
"""
import pandas as pd
import numpy as np
