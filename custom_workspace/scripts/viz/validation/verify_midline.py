"""
Midline correction debug tool — wrist X vs sternum X alignment check.

What it does:
  Loads a TRC file, extracts the wrist (V_Wrist) and sternum (CS) marker X
  trajectories, and plots them overlaid. Used to verify the midline correction
  in convert_trc2mot.py is working: wrist X should converge to sternum X at
  the drinking peak (hand-to-mouth moment).

Input:
  - output/generated/trc/FMA_XX.trc  (or any .trc file passed as CLI arg)

Output:
  - output/generated/plots/verify_midline.png  — wrist X vs sternum X plot

Usage:
  python scripts/viz/validation/verify_midline.py                          # FMA_50
  python scripts/viz/validation/verify_midline.py output/generated/trc/FMA_30.trc
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
