"""
EMG validation — automated pass/fail checks vs published activation literature.

What it does:
  Loads activations.csv and effort_metrics.json for every FMA level in id_base_dir,
  runs 5 checks per session against published EMG reference values, and prints a
  pass/fail report with totals.

Input:
  - output/generated/id/<FMA_XX>/activations.csv      (32 muscles × frames)
  - output/generated/id/<FMA_XX>/effort_metrics.json  (CCI scalar)
  - output/generated/id/<FMA_XX>/phase_indices.json   (Pick/Drink/Place frame indices)

Output:
  - Console: per-check pass/fail table, summary counts
  - No file written (pipe to a file if you want to save: ... > report.txt)

Protocol (5 checks × N sessions):
  1. Reach-phase dominant muscles ≥60% overlap with {DELT, BIC} (Alt Murphy 2011)
  2. Drink-phase dominant muscles ≥60% overlap with {BIC, DELT} (Alt Murphy 2011)
  3. Place-phase dominant muscles ≥60% overlap with {TRI} (Alt Murphy 2011)
  4. CCI in published range 0.2–0.6 (healthy) / 0.4–0.8 (impaired) (Roh 2013)
  5. 4-synergy NMF explains ≥90% VAF (Roh 2013)

Usage:
  python scripts/viz/validation/validate_emg.py
  python scripts/viz/validation/validate_emg.py output/generated/id/
"""
import os
import sys
import re
