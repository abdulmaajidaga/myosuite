"""
SMOTE augmentation quality check — 3 diagnostic figures.

What it does:
  Loads a random subsample of SMOTE-augmented files, computes PCA, wrist
  trajectories, and kinematic metrics to confirm the augmentation produced a
  smooth FMA gradient and realistic motion.

Input:
  - data/kinematic/cutoff/augmented_smote/*.csv  (SMOTE-augmented, 56k files)

Output:
  - output/generated/plots/smote_pca.png          — PCA colored by FMA score
  - output/generated/plots/smote_trajectories.png — wrist Y trajectories FMA 16→66
  - output/generated/plots/smote_metrics.png      — kinematic metric trends

Usage:
  python scripts/viz/figures/visualize_smote.py
"""
import numpy as np
import pandas as pd
