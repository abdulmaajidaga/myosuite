"""
Cross-FMA inverse dynamics comparison — pipeline-importable plotting module.

What it does:
  Scans an id_base_dir for per-FMA subdirectories, loads joint_torques.csv and
  activations.csv from each, and produces multi-panel overlay figures comparing
  all FMA levels on the same axes. Called automatically at the end of
  run_generated_pipeline.py and calc_mot2invdyn.py.

Input:
  - id_base_dir/<FMA_XX>/joint_torques.csv    (7 joints × frames)
  - id_base_dir/<FMA_XX>/activations.csv      (32 muscles × frames)
  - id_base_dir/<FMA_XX>/effort_metrics.json  (ATI, CCI scalars)

Output:
  - output_dir/cross_fma_torques.png      — torque overlays per joint
  - output_dir/cross_fma_activations.png  — activation overlays per muscle group
  - output_dir/cross_fma_summary.png      — bar chart of ATI/CCI vs FMA

Protocol:
  Imported as `from scripts.viz.figures.plot_id_comparison import generate_all`
  by scripts/run_generated_pipeline.py and src/inverse_dynamics/calc_mot2invdyn.py.

Usage:
  python scripts/viz/figures/plot_id_comparison.py   # standalone, uses default paths
"""
import os
import re
import sys
