"""
FMA trend analysis — generates motions across the full stroke-to-healthy range.

What it does:
  Uses the trained CVAE to generate one motion per integer FMA score from 16→66,
  then plots wrist range, velocity, trunk compensation, and clinical metric trends.
  Produces two figures and a data CSV.
...
Output:
  - output/generated/plots/fma_trend_graphs.png    — per-metric trend curves FMA 16→66
  - output/generated/plots/fma_trend_summary.png   — summary panel
  - output/generated/plots/fma_trend_data.csv      — raw metric values

Usage:
  python scripts/viz/figures/fma_trend_analysis.py
"""
import torch
