"""
Trunk compensation analysis: stroke vs healthy comparison.

What it does:
  Loads all processed cutoff CSVs (15-DOF delta format), extracts the 3 trunk
  columns (Trunk_x/y/z), computes per-subject trunk displacement and range, then
  produces a 2-row figure: boxplots comparing stroke vs healthy, and per-subject
  scatter against FMA score.

Input:
  - data/kinematic/cutoff/processed/*.csv  (15-col delta format, 100 frames)
  - output/scores.csv                      (FMA score per subject filename)

Output:
  - figures/trunk_compensation.png         (2×4 panel figure; LaTeX-referenced)

Usage:
  python scripts/viz/figures/analyze_trunk_compensation.py
"""
import pandas as pd
import numpy as np
...
# --- Configuration ---
HEALTHY_DIR = get_path("data_healthy")
STROKE_DIR = get_path("data_stroke")
