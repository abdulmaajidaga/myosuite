"""
ID findings figures — 7 publication-quality panels from inverse dynamics output.

What it does:
  Loads all effort_metrics.json and activations.csv files produced by
  calc_mot2invdyn.py across every FMA level, then generates 7 figures covering
  muscle dominance, activation profiles, phase proportions, torque severity gradients,
  synergy timing, biceps/triceps co-contraction, and a summary dashboard.

Input:
  - output/generated/id/<FMA_XX>/effort_metrics.json   (ATI, CCI, TRR, synergy VAF)
  - output/generated/id/<FMA_XX>/activations.csv       (32 muscles × frames)
  - output/generated/id/<FMA_XX>/joint_torques.csv     (7 joints × frames)
  - output/generated/id/<FMA_XX>/phase_indices.json    (Pick/Drink/Place frame indices)

Output (all in output/generated/plots/findings/):
  - muscle_dominance.png     — top-10 muscles per FMA (LaTeX-referenced)
  - activation_profiles.png  — muscle group time-series with phase shading
  - phase_proportions.png    — stacked bars + pie charts for Pick/Drink/Place
  - torque_profiles.png      — joint torques with FMA severity gradient
  - synergy_timing.png       — synergy coefficient heatmaps + temporal peaks
  - cocontraction.png        — biceps/triceps CCI over motion
  - summary_dashboard.png    — single-page overview (LaTeX-referenced)

Usage:
  python scripts/viz/figures/visualize_id_findings.py
"""
import numpy as np
import pandas as pd
import matplotlib
