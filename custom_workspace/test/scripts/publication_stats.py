...
def group_comparisons(df):
    """Healthy (FMA 50+66) vs Impaired (FMA 18+20+30): Mann-Whitney U + Cohen's d."""
    healthy_mask = df['fma_score'] >= 50
    impaired_mask = df['fma_score'] <= 30

    healthy = df[healthy_mask]
    impaired = df[impaired_mask]

    lines = [f'## C. Group Comparisons\n']
    lines.append(f'Healthy group: FMA >= 50 (N={len(healthy)})')
    lines.append(f'Impaired group: FMA <= 30 (N={len(impaired)})\n')
...
def main():
    ...
    impaired_mask = gen_df['fma_score'] <= 30
