    ...
    if os.path.isdir(gen_id_dir):
        for name in sorted(os.listdir(gen_id_dir)):
            ...
            with open(path) as f:
                data = json.load(f)
            data['session'] = name
            data['fma_score'] = int(m.group(1))
            data['source'] = 'generated'
            rows.append(data)

    # Original sessions — use actual FMA scores from scores.csv
...
def main():
    parser = argparse.ArgumentParser(description="Literature Validation Suite")
    parser.add_argument("--output-dir", type=str, default=None, help="Directory to save report/plots")
    parser.add_argument("--input-id-dir", type=str, default=None, help="Directory containing generated ID results")
    args = parser.parse_args()

    if args.input_id_dir:
        gen_id_dir = args.input_id_dir
    else:
