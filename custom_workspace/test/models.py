"""
models.py — Configurable CVAE for stroke motion generation.

Single class, controlled via a config dict:
  use_film          (bool): FiLM conditioning vs simple concatenation
  use_residual      (bool): Residual skip connection in decoder
  use_cfg           (bool): Classifier-free guidance support
  use_temporal_conv (bool): TemporalConvBlock after LSTM (1D CNN refinement)
...
Architecture map:
  ...
  Stage 2 equivalent: use_film=True,  use_cfg=True,  use_residual=False
...
class TemporalConvBlock(nn.Module):
    ...
    Ported from root model_v2.py. Sits after the LSTM in the decoder and
    learns FMA-dependent temporal shaping: wide smooth patterns for healthy,
    sharper/jerkier patterns for severe stroke.

    Uses a bottleneck (hidden→64→64→hidden) so parameter count stays low.
    Kernel=5 gives a ±2-frame receptive field.
    """
...
class MotionCVAE(nn.Module):
    ...
    def __init__(self, config: dict = None):
        super().__init__()
        cfg = config or {}
        self.use_film          = cfg.get('use_film',          False)
    ...
    # ── inference ────────────────────────────────────────────────────
    def inference(self, c, seq_len: int = SEQ_LEN, guidance_scale: float = 1.0):
        with torch.no_grad():
