...
# ========================================
INPUT_CSV = os.path.join(get_path("data_healthy"), "01_12_1.csv")
OUTPUT_TRC = os.path.join(get_path("output_dir"), "01_12_1.trc")
DATA_RATE = 200.0
VISUALIZE = False
SAVE_PLOT = None
...
def calculate_virtual_joints(df):
    """
    Calculates the virtual joint centers (Shoulder, Elbow, Wrist) and a
    V_Vector marker for wrist rotation from the anatomical markers.
    """
    ...
    processed_df[('V_Shoulder', 'Z')] = (processed_df['SA_1']['Z'] + processed_df['SA_3']['Z'] + processed_df['SA_3']['Z']) / 3

    # V_Vector: Wrist center + 0.5 * (WRB - WRA), gives wrist rotation reference
    processed_df[('V_Vector', 'X')] = processed_df[('V_Wrist', 'X')] + 0.5 * (processed_df['WRB']['X'] - processed_df['WRA']['X'])
    processed_df[('V_Vector', 'Y')] = processed_df[('V_Wrist', 'Y')] + 0.5 * (processed_df['WRB']['Y'] - processed_df['WRA']['Y'])
    processed_df[('V_Vector', 'Z')] = processed_df[('V_Wrist', 'Z')] + 0.5 * (processed_df['WRB']['Z'] - processed_df['WRA']['Z'])
...
def save_to_trc(df, trc_filepath, data_rate=200):
    ...
    """
    markers_to_save = ['V_Shoulder', 'V_Elbow', 'V_Wrist', 'V_Vector']
